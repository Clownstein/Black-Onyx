...
class BoomPipeline:
    def process_events(self, events: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        raise ValueError("bad code event")


class OkPipeline:
    def process_events(self, events: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
        return ([{"feature_version": "code.features.v1", "events": events}], [])


def test_handle_message_publishes_to_dlq_on_failure() -> None:
    consumer = CodeConsumer(pipeline=BoomPipeline())  # type: ignore[arg-type]
    ...
    assert envelope["error"] == "bad code event"
    assert envelope["payload"] == payload


def test_handle_message_publishes_features_on_success() -> None:
    consumer = CodeConsumer(pipeline=OkPipeline())  # type: ignore[arg-type]
    producer = FakeProducer()
    consumer._producer = producer  # type: ignore[assignment]
    ...
    assert feature["feature_version"] == "code.features.v1"
