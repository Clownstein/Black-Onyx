...
from code_processor.pipeline import CodePipeline

logger = logging.getLogger(__name__)


class CodeConsumer:
    def __init__(self, pipeline: CodePipeline | None = None) -> None:
        self.pipeline = pipeline or CodePipeline()
    ...
    def start(self) -> None:
        if not settings.enable_kafka:
            self.ready = True
            return
        self._thread = threading.Thread(target=self._run, name="code-consumer", daemon=True)
        self._thread.start()
    ...
    def _handle_message(self, event: dict[str, Any]) -> None:
        try:
            ...
            logger.exception("failed to process code message; sending to DLQ")
    ...
    def _run(self) -> None:
        try:
            ...
            self.last_error = str(exc)
            self.ready = False
            logger.exception("code consumer failed")

    def process_batch(self, events: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
