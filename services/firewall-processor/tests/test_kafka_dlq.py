...
class BoomPipeline:
    def process_events(
        ...
        raise ValueError("bad firewall event")
...
def test_handle_message_publishes_to_dlq_on_failure() -> None:
    consumer = FirewallConsumer(pipeline=BoomPipeline())  # type: ignore[arg-type]
    ...
    assert envelope["error"] == "bad firewall event"
    assert envelope["payload"] == payload


def test_handle_message_publishes_features_and_findings_on_success() -> None:
    consumer = FirewallConsumer(pipeline=OkPipeline())  # type: ignore[arg-type]
    producer = FakeProducer()
    consumer._producer = producer  # type: ignore[assignment]
