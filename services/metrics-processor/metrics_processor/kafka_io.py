...
from metrics_processor.pipeline import MetricsPipeline

logger = logging.getLogger(__name__)


class MetricsConsumer:
    def __init__(self, pipeline: MetricsPipeline | None = None) -> None:
        self.pipeline = pipeline or MetricsPipeline()
    ...
    def start(self) -> None:
        if not settings.enable_kafka:
            self.ready = True
            return
        self._thread = threading.Thread(target=self._run, name="metrics-consumer", daemon=True)
        self._thread.start()
    ...
    def _handle_message(self, event: dict[str, Any]) -> None:
        try:
            ...
            logger.exception("failed to process metrics message; sending to DLQ")
    ...
    def _run(self) -> None:
        try:
            ...
            self.last_error = str(exc)
            self.ready = False
            logger.exception("metrics consumer failed")
