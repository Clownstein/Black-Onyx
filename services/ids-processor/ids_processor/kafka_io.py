...
class IdsConsumer:
    def __init__(self, pipeline: IdsPipeline | None = None) -> None:
        self.pipeline = pipeline or IdsPipeline()
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run, name="ids-consumer", daemon=True
        )
        self._thread.start()
    ...
    def _handle_message(self, event: dict[str, Any]) -> None:
        try:
            findings = self.pipeline.process_events([event])
            if self._producer is None:
                return
            ...
            logger.exception("failed to process ids message; sending to DLQ")
    ...
    def _run(self) -> None:
        try:
            ...
            self.last_error = str(exc)
            self.ready = False
            logger.exception("ids consumer failed")
