...
class FlowConsumer:
    def __init__(self, pipeline: FlowPipeline | None = None) -> None:
        self.pipeline = pipeline or FlowPipeline()
    ...
    def start(self) -> None:
        if not settings.enable_kafka:
            self.ready = True
            return
        self._thread = threading.Thread(target=self._run, name="flow-consumer", daemon=True)
        self._thread.start()
    ...
    def _handle_message(self, event: dict[str, Any]) -> None:
        try:
            features = self.pipeline.process_events([event])
            if self._producer is None:
                return
            ...
            logger.exception("failed to process flow message; sending to DLQ")
            self._publish_dlq(event, exc)

    def _run(self) -> None:
        try:
            brokers = [b.strip() for b in settings.kafka_brokers.split(",") if b.strip()]
            topics = [settings.topic_raw]
            if settings.consume_zeek_raw and settings.topic_zeek_raw:
                topics.append(settings.topic_zeek_raw)
            if settings.consume_dns_raw and settings.topic_dns_raw:
                topics.append(settings.topic_dns_raw)
            ...
            self.last_error = str(exc)
            self.ready = False
            logger.exception("flow consumer failed")
