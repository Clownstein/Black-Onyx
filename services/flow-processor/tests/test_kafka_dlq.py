...
class BoomPipeline:
    def process_events(self, events: list[dict[str, Any]]) -> list[dict[str, Any]]:
        raise ValueError("bad flow event")


class OkPipeline:
    def process_events(self, events: list[dict[str, Any]]) -> list[dict[str, Any]]:
        return [{"feature_version": "network.features.v1", "events": events}]


def test_handle_message_publishes_to_dlq_on_failure() -> None:
    consumer = FlowConsumer(pipeline=BoomPipeline())  # type: ignore[arg-type]
    ...
    assert envelope["error"] == "bad flow event"
    assert envelope["payload"] == payload


def test_handle_message_publishes_features_on_success() -> None:
    consumer = FlowConsumer(pipeline=OkPipeline())  # type: ignore[arg-type]
    producer = FakeProducer()
    consumer._producer = producer  # type: ignore[assignment]
    ...
    assert feature["feature_version"] == "network.features.v1"
