...
from host_state_processor.heartbeat import HeartbeatMonitor
from host_state_processor.pipeline import HostStatePipeline

logger = logging.getLogger(__name__)


class HostStateConsumer:
    def __init__(self, pipeline: HostStatePipeline | None = None) -> None:
        self.pipeline = pipeline or HostStatePipeline()
        ...
        self.heartbeat = HeartbeatMonitor(
            last_seen_provider=self.pipeline.last_seen_snapshot,
            publish=self._publish_finding,
        )

    def _publish_finding(self, finding: dict[str, Any]) -> None:
        """Send a finding to the findings topic (used by the heartbeat monitor)."""
    ...
    def _handle_message(self, event: dict[str, Any]) -> None:
        try:
            ...
            logger.exception("failed to process host-state message; sending to DLQ")
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run, name="host-state-consumer", daemon=True
        )
        self._thread.start()
    ...
    def _run(self) -> None:
        try:
            ...
            self.last_error = str(exc)
            self.ready = False
            logger.exception("host-state consumer failed")
