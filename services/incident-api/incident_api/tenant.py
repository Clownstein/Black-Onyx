"""Tenant auth, optional OIDC JWT validation, and RBAC roles."""
...
# Auditor is read-only assessor access (same rank as viewer).
...
def require_principal(
    ...
    if _constant_time_eq(x_service_key, settings.service_api_key):
        return Principal(
            tenant_id=tenant_id,
            subject="service",
            roles={"admin"},
    ...
    # Prefer Bearer JWT when HS/OIDC is configured (Black Onyx BFF mint path).
    if (
        authorization
